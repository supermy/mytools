...
docker exec flume_mq_1 bash -c 'rabbitmqctl add_vhost statuscheckvhost'
docker exec flume_mq_1 bash -c 'rabbitmqctl add_user heartbeat alive'
docker exec flume_mq_1 bash -c 'rabbitmqctl set_permissions -p statuscheckvhost heartbeat ".*" ".*" ".*"'
docker exec flume_mq_1 bash -c 'rabbitmqctl set_user_tags heartbeat management'
docker exec flume_mq_1 bash -c 'curl -i -u heartbeat:alive http://127.0.0.1:15672/api/aliveness-test/statuscheckvhost'
