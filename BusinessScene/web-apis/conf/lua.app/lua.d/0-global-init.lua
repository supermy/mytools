...
--加载配置文件
local file = io.open("/usr/local/openresty/lua.app/query.d/0-config.json", "r");
...
local fileDbCodis = io.open("/usr/local/openresty/lua.app/query.d/0-config-db-codis.json", "r");
local contentDbCodis = fileDbCodis:read("*all");
fileDbCodis: close();

myconfig: set("cfgDbCodis", contentDbCodis);

local fileDbAcl = io.open("/usr/local/openresty/lua.app/query.d/0-config-db-acl.json", "r");
...
local fileDbQuery = io.open("/usr/local/openresty/lua.app/query.d/0-config-db-query.json", "r");
...
function set_to_cache(key, value, exptime)
    ...
    local cache_ngx = ngx.shared.myconfi
...
--dofile("/usr/local/lua.app/waf.d/init.lua")